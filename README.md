...
- **Auto-allow all repositories** — 开启后自动批准所有检测到的 GitHub 确认弹窗，首次开启会要求二次确认
- **仓库列表** — 添加 `owner/repository` 格式的仓库，仅对这些仓库自动批准
- **Clear remembered approvals** — 清除已记住的信任规则，执行前会要求确认
...
- 清除 remembered approvals 前会要求二次确认
...
- 全仓库自动批准和清除 remembered approvals 都会要求用户二次确认。
...
| `shared.js` | 共享工具：默认仓库、仓库/规则规范化与校验 |
...
- 新增 `shared.js`，集中默认仓库、仓库规范化、规则规范化等共享逻辑
- `background.js`、`popup.js`、`options.js`、`content.js` 复用共享规则工具，减少后续漏改风险
- 开启全仓库自动批准前增加二次确认
- 清空 remembered approvals 前增加二次确认
